  ...
  return <section className="configurator-section" id="services">
    <div className="section-shell">
      ...
      <div className="configurator-grid reveal">
        <div className="service-selector" role="group" aria-label="Choose a service">{services.map((service) => <button type="button" className={`service-option ${selectedService === service.title ? 'is-active' : ''}`} aria-pressed={selectedService === service.title} onClick={() => onServiceChange(service.title)} key={service.title}><span><strong>{service.title}</strong><small>{service.description}</small></span><Check className="service-check" size={20} /></button>)}</div>
